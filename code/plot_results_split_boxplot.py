...
fig, axes = plt.subplots(n_rows, n_cols, figsize=(20, 10), sharex=False, sharey=True)
...
for idx, node_id in enumerate(nodes_to_plot):
    ...
    axes[idx].legend(
        handles=[
            plt.Line2D([0], [0], color="blue", lw=4, alpha=0.5, label="Original"),
            plt.Line2D([0], [0], color="orange", lw=4, alpha=0.5, label="Merged"),
            plt.Line2D([0], [0], color="green", lw=4, alpha=0.5, label="Naive")
        ],
        loc="upper right"
    )
...
plt.suptitle(f'Boxplots of MSE for Different Nodes for {target}', fontsize=16)
plt.savefig(f'../figures/results-split-boxplot.pdf', format='pdf', bbox_inches='tight')
plt.show()
